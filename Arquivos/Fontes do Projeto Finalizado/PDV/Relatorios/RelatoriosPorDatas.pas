...
begin
      ...
      dm.rel_Mov_Saidas.SQL.Add('select sum(valor) as total from movimentacoes where data >= :dataInicial and data <= :dataFinal and tipo = "Saída" ') ;
...
begin
       ...
       dm.query_vendas.SQL.Add('SELECT * from vendas where data >= :dataInicial and data <= :dataFinal and status = :status order by data desc');
       ...
       dm.query_vendas.ParamByName('dataFinal').Value := FormatDateTime('yyyy/mm/dd' ,dataFinal.Date);
        dm.query_vendas.ParamByName('status').Value := cbStatus.Text;
       dm.query_vendas.Open;

     dm.rel_vendas.LoadFromFile(GetCurrentDir + '\rel\vendas.fr3');
     dm.rel_vendas.Variables['dataInicial'] := dataInicial.Date;
     dm.rel_vendas.Variables['dataFinal'] := dataFinal.Date;
     dm.rel_vendas.Variables['status'] :=  '''' + cbStatus.Text + '''';
     dm.rel_vendas.ShowReport();
end;
